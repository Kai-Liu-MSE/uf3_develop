...
class WeightedLinearModel(BasicLinearModel):
    ...
    def set_frozen_coefficients(self, indices, values=None):
        """Freeze selected coefficient columns before fitting."""
        indices = np.array(indices, dtype=int)
        if values is None:
            values = np.zeros(len(indices))
        values = np.array(values, dtype=float)
        if len(indices) != len(values):
            raise ValueError("Frozen index and value counts do not match.")
        keep = {}
        for idx, value in zip(self.col_idx, self.frozen_c):
            keep[int(idx)] = float(value)
        for idx, value in zip(indices, values):
            keep[int(idx)] = float(value)
        ordered = sorted(keep)
        self.bspline_config.col_idx = np.array(ordered, dtype=int)
        self.bspline_config.frozen_c = np.array([keep[idx] for idx in ordered])

    def freeze_feature_groups(self, groups, coefficients=None):
        """
        Freeze named feature groups.

        If coefficients are provided, the selected groups are frozen to those
        values.  Otherwise they are frozen to zero, which is convenient for
        residual fits that train only a correction block.
        """
        indices = self.bspline_config.get_feature_group_indices(groups)
        if coefficients is None:
            values = np.zeros(len(indices))
        else:
            values = np.asarray(coefficients)[indices]
        self.set_frozen_coefficients(indices, values)

    def initialize_from_model(self, model):
        """
        Copy coefficients from another UF3 linear model by interaction key.

        This is useful for residual workflows where a baseline UF3 model is
        transferred into an expanded basis containing extra correction blocks.
        Missing blocks in the source model are initialized to zero by load().
        """
        solution = arrange_coefficients(model.coefficients,
                                        model.bspline_config)
        for trio in model.bspline_config.interactions_map.get(3, []):
            solution[trio] = model.bspline_config.decompress_3B(solution[trio],
                                                                trio)
        self.load(solution=solution)
    ...
    def load(self,
        ...
        solution = normalize_scalar_descriptor_keys(solution)
        scalar_prefixes = {
            "density",
            "dipole",
            "shell_density",
            "density_moment",
            "local_composition",
            "density_modulated_2b",
            "density_modulated_3b",
            "dipole_modulated_2b",
        }
        for key in list(solution):
            if isinstance(key, tuple) and key[0] not in scalar_prefixes:
        ...
        if self.bspline_config.density_enabled:
            for interaction in self.bspline_config.density_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
                n_provided = len(np.atleast_1d(solution[interaction]))
                if n_provided != n_target:
                    raise ValueError(
                        f"Incorrect shape: {interaction}, "
                        f"{n_provided} != {n_target}"
                    )
        if self.bspline_config.dipole_enabled:
            for interaction in self.bspline_config.dipole_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.shell_density_enabled:
            for interaction in self.bspline_config.shell_density_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.density_moment_enabled:
            for interaction in self.bspline_config.density_moment_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.local_composition_enabled:
            for interaction in self.bspline_config.local_composition_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.density_modulated_2b_enabled:
            for interaction in self.bspline_config.density_modulated_2b_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.density_modulated_3b_enabled:
            for interaction in self.bspline_config.density_modulated_3b_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.dipole_modulated_2b_enabled:
            for interaction in self.bspline_config.dipole_modulated_2b_interactions:
                n_target = component_len[interaction]
                if interaction not in solution:
                    warnings.warn(f"{interaction} not provided.")
                    solution[interaction] = np.zeros(n_target)
        ...
        if self.bspline_config.density_enabled:
            for interaction in self.bspline_config.density_interactions:
                values = np.atleast_1d(solution[interaction])
...
def arrange_coefficients(coefficients, bspline_config):
    ...
    j = 0
    if bspline_config.density_enabled:
        for interaction in bspline_config.density_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.dipole_enabled:
        for interaction in bspline_config.dipole_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.shell_density_enabled:
        for interaction in bspline_config.shell_density_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.density_moment_enabled:
        for interaction in bspline_config.density_moment_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.local_composition_enabled:
        for interaction in bspline_config.local_composition_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.density_modulated_2b_enabled:
        for interaction in bspline_config.density_modulated_2b_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.density_modulated_3b_enabled:
        for interaction in bspline_config.density_modulated_3b_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
    if bspline_config.dipole_modulated_2b_enabled:
        for interaction in bspline_config.dipole_modulated_2b_interactions:
            solutions[interaction] = solutions_list[j]
            j += 1
...
def normalize_scalar_descriptor_keys(solution):
    """Normalize JSON-loaded scalar descriptor tuple keys."""
    normalized = {}
    for key, value in solution.items():
        if not isinstance(key, tuple) or not key:
            ...
            continue
        prefix = key[0]
        if prefix == "shell_density" and len(key) == 4:
            normalized[(prefix, int(key[1]), str(key[2]), str(key[3]))] = value
        elif prefix == "density_moment" and len(key) == 4:
            normalized[(prefix, float(key[1]), str(key[2]), str(key[3]))] = value
        elif prefix in {"density", "dipole", "local_composition"} and len(key) == 3:
            normalized[(prefix, str(key[1]), str(key[2]))] = value
        elif prefix == "density_modulated_2b" and len(key) == 4:
            normalized[(prefix, str(key[1]), str(key[2]), str(key[3]))] = value
        elif prefix == "dipole_modulated_2b" and len(key) == 3:
            normalized[(prefix, str(key[1]), str(key[2]))] = value
        else:
            normalized[key] = value
    return normalized
